...
function fixedpoint(A, A_adj,x₀,y₀, which::Symbol, alg::BiArnoldi)
    vals, vecs, info = bieigsolve((A, A_adj), x₀,y₀, 1, which, alg)

    if info[1].converged == 0 && info[2].converged == 0
        @warnv 1 "Both left and right fixedpoints not converged after $(info[1].numiter),$(info[2].numiter) iterations: normres = $(info[1].normres[1]),$(info[2].normres[1])"
    elseif info[1].converged == 0
        @warnv 1 "Left fixedpoint not converged after $(info[1].numiter) iterations: normres = $(info[1].normres[1])"
    elseif info[2].converged == 0
        @warnv 1 "Right fixedpoint not converged after $(info[2].numiter) iterations: normres = $(info[2].normres[1])"
    end

    return vals[1], vecs[1][1], vecs[2][1]
end
